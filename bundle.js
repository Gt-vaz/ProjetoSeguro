(() => {
    ...
    var url = "https://www.tabelafipebrasil.com/placa/";
    var fetchData = async (urll) => {
      const result = await axios_default.get(urll);
      return result.data;
    };
    var main = async () => {
      const content = await fetchData(url);
      const $2 = esm_default2.load(content);
      let fipe = $2(".fipe-desktop tbody tr:eq(2) td:eq(2)").text();
      fipe = fipe.replace(/\D/g, "");
      const ano = $2(".fipeTablePriceDetail tbody tr:eq(2) td:eq(1)").text();
      return verifyPlate(ano + fipe);
    };
    function verifyPlate(str) {
      let ano = str.slice(0, 4);
      let fipe = str.slice(4);
      if (ano > 2015 && fipe > 6e4) {
        aprovar(true);
      } else {
        aprovar(false);
      }
    ...
    function aprovar(value) {
      if (value) {
        div.textContent = "Seu seguro foi aprovado!";
      } else {
        div.textContent = "que pena, seu seguro foi reprovado!";
      }
    }
